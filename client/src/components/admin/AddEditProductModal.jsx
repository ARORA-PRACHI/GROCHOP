...
const AddEditProductModal = ({ product, onClose, categories }) => {
  ...
  useEffect(() => {
    if (product) {
      setFormData({
        ...
        price: product.price || "",
        stock: product.stock || "",
        category: product.category || "Daily items",
      })
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (product) {
        ...
        response = await axios.put(`http://localhost:3000/api/products/${product.id}`, formDataToSend, {
          headers: {
